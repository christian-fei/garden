...
const { env: { OPEN_WEATHER_MAP_API_KEY, LAT, LON } } = process

exports.gatherReport = (chat) => {
  let history = []
  try {
    history = temperatureMoistureHistory.read()
  } catch (err) {
    console.error('failed to read temperatureMoistureHistory', err.message, err)
  }
...
function iso (date = new Date(), tz = 0) {
